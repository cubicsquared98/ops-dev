Shader "cubic/ops/tools/resolution_checker"
{
    Properties
    {
        // _ID ("Channel to read", Int) = 0
        _IntDigits ("Integer Digits", Range(1, 6)) = 2
        _DecDigits ("Decimal Digits", Range(0, 4)) = 0
        _Color ("Text Color", Color) = (0, 1, 0, 1)
        _BgColor ("Background Color", Color) = (0, 0, 0, 1)

        _OPS_TYPE ("Hole Type (0 == orifice, 1 == penetrator)", Int) = 0
    }
...
fixed4 frag (v2f i) : SV_Target
            {
                ...
                //float3 read_data = readDataFrom_ID_TEX(uint2(0,0)); //Read from the bottom left of the screen. Max ID overlaps occur here.

                //float Total_Ids = read_data.x * getMultiplierDecode();

                float totalWidth = _ScreenParams.x;
                float totalHeight = _ScreenParams.y;

                float g_totalWidth = _OPS_ID_HASH_TEXTURE_TexelSize.z;
                float g_totalHeight = _OPS_ID_HASH_TEXTURE_TexelSize.w;

                float r_totalWidth;
                float r_totalHeight;

                _OPS_ID_HASH_TEXTURE.GetDimensions(r_totalWidth, r_totalHeight);



                float other_values = 0;

                // float3 values   = readInlineFloat3Data(offset_world_pos, _ID);
                // float3 values_2 = readInlineFloat3Data(offset_world_forward_vec, _ID);
                // float3 values_3 = readInlineFloat3Data(offset_world_up_vec, _ID);
                // float  value4   = readInlineFloatData(offset_ops_type, _ID);
                ...
                float rawVal = 0;
                if      (currentRow == 9) rawVal = totalWidth;      // The 10th value
                else if (currentRow == 8) rawVal = totalHeight;      // The 9th value
                else if (currentRow == 7) rawVal = g_totalWidth;      // The 8th value
                else if (currentRow == 6) rawVal = g_totalHeight;      // The 7th value
                else if (currentRow == 5) rawVal = r_totalWidth;      // The 8th value
                else if (currentRow == 4) rawVal = r_totalHeight;      // The 7th value
                else                      rawVal = other_values; 
