Shader "cubic/ops/tools/penetrator_debug_reader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                #if defined(UNITY_STEREO_INSTANCING_ENABLED) || defined(UNITY_SINGLE_PASS_STEREO) || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
                    ScreenWidth *= 0.5;
                #endif
                float HalfWidth = ScreenWidth * 0.5;

                uint4 __ID_space = getIDSpace(ID_SPACE_PENETRATOR);

                float3 __read_data = readDataFrom_ID_TEX(__ID_space.zw);

                int __Total_Ids = min(_OPS_TextureExists() ? round(__read_data.r * getMultiplierDecode()): 0, 2000); //If no ops texture, will instantly fallback to sps search


                // Read all data streams
                float3 values   = readInlineFloat3Data(HalfWidth + offset_penetrator_world_start_point_x, _ID);
                float3 values_2 = readInlineFloat3Data(HalfWidth + offset_penetrator_world_end_point_x, _ID);
                float3 values_3 = readInlineFloat3Data(HalfWidth + offset_penetrator_world_radius_up_point_x, _ID);
                //float  value4   = float(readInlineUintData(HalfWidth + offset_penetrator_bone_data_start_bone_index, _ID));
                float  value4   = __Total_Ids;//readInlineUintData(HalfWidth + offset_penetrator_frot_mode, _ID); //radius X
                //distance(values, values_2);
