...
void sps_light_search_only(float3 local_search_pos, inout float3 found_root_normal, inout float3 found_root_position, inout int found_type)
{
	//Reset validity of incomming orifice.
	found_type = SPS_TYPE_INVALID;
	...
	bool rootFound = false;
	{
		float minDistance = -1;
		for(int i = 0; i < 4; i++) {
			const float distance = length(lightLocalPos[i] - local_search_pos);
	...
	if (!rootFound) return;
	if (found_type != SPS_TYPE_INVALID && length(lightLocalPos[rootIndex]) >= length(found_root_position)) return;

	found_type = lightType[rootIndex];
	found_root_position = lightLocalPos[rootIndex];
	found_root_normal = frontFound
		? lightLocalPos[frontIndex] - lightLocalPos[rootIndex]
		: -1 * lightLocalPos[rootIndex];
	found_root_normal = sps_normalize(found_root_normal);
}
